...
export function VariantSection({ productId, productName }: { productId: string; productName: string }) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
      <div>
        <h2 className="font-semibold text-gray-900">Variantes</h2>
        <p className="text-xs text-muted-foreground mt-0.5">
          Talles, sabores, colores u otras opciones de <span className="font-medium">{productName}</span>
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {variants.map(v => (
            <div
              key={v.id}
              className={`flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 ${!v.isActive && 'opacity-50'}`}
            >
